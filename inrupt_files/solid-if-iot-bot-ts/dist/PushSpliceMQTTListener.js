...
import { saveSolidDatasetAt, buildThing, getSolidDataset, createThing, setThing, getStringNoLocale, getThingAll, universalAccess } from "@inrupt/solid-client";
import EventEmitter from "events";
import { DEFAULT_PAYLOAD_URI, DEFAULT_SENSOR_CONTACTS_URI_PATH, DEFAULT_SENSOR_CONTAINER_STRING, DEFAULT_SUBSCRIBED_TOPICS_URI_PATH, DEFAULT_TOPIC_URI, DEFAULT_WEBID_CONTACT_IRI } from "./constants.js";
import { extractStringsFromThing, getAllContainedResourcesInDataset, getAllThingsWithIri, getDataOrCreateDataset, getProfile, getStorageUri, getWebIdGraph, wsNotificationBuilder } from "./utils.js";
export class PushSpliceMqttListener {
    constructor(session, dataUriPath, pushShiftSize) {
        // maybe use the store when the streams are getting overwhelming?
        // private auxStore: mqtt.Store;
        this.auxStore = [];
        this.counter = 0;
        this.shift_counter = 0;
        this.mqttClientCache = {};
    ...
    ;
    async setup(sensorContactsUriPath = DEFAULT_SENSOR_CONTACTS_URI_PATH, subscribedTopicsUriPath = DEFAULT_SUBSCRIBED_TOPICS_URI_PATH) {
        this.webId = this._session.info.webId;
        this._storageUri = await getStorageUri(this.webId, this._session.fetch);
        ...
        this.sensorContactsCache = await getAllThingsWithIri(this._sensorContactsUri, DEFAULT_WEBID_CONTACT_IRI, this._session.fetch);
        this._graph = await getWebIdGraph(this.webId, this._session.fetch);
        this._profile = await getProfile(this.webId, this._graph, this._session.fetch);
        this._sensorContainerResourceUri = getStringNoLocale(this._profile, DEFAULT_SENSOR_CONTAINER_STRING);
    ...
    buildMqttCache() {
        for (const st of this.subscribedTopicsCache) {
            ...
            const iqos = qos ? Number.parseInt(qos) : 0;
    ...
    initSensorContactsSocket() {
        ...
        this.sensorContactsSocket.on("message", async (notif) => {
            let newSensorContactsCache = await getAllThingsWithIri(this._sensorContactsUri, DEFAULT_WEBID_CONTACT_IRI, this._session.fetch);
            ...
            this.sensorContactsCache = newSensorContactsCache;
        });
        ...
        this.subscribedTopicsSocket.on("message", async (notif) => {
            ...
            if (newSubscribedTopicsCache.length < this.subscribedTopicsCache.length) {
                ...
                for (const st of topicsToSubscribe) {
                    ...
                    const cacheIdentifier = `${topicsUri}+${topicName}`;
                    if (this.mqttClientCache[cacheIdentifier]) {
                        this.mqttClientCache[cacheIdentifier].subscribe(topicName, { qos: Number.parseInt(qos) }, (err, packet) => {
                            if (err)
                                console.log(err);
                            console.log(packet);
                        });
                        ...
                        let targetMqttClient = mqtt.connect(brokerUri);
                        targetMqttClient.subscribe(topicName, { qos: Number.parseInt(qos) }, (err, packet) => {
                            if (err)
                                console.log(err);
                            console.log(packet);
                        });
