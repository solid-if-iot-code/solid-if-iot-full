...
export class PushSpliceMqttListener {
    ...
    constructor(dataUriPath: string, pushShiftSize: number) {
        ...
        this.dataBuffer = [];
        this.push_shift_size = pushShiftSize;
    ...
    async setup
        ...
        subscribedTopicsUriPath: string = DEFAULT_SUBSCRIBED_TOPICS_URI_PATH,
            ...
            // TODO: the provider base uri needs to be a .env variable
        const credentialResponse = await fetch('http://localhost:3000/idp/credentials/', {
            method: 'POST',
            headers: { 'content-type': 'application/json' },
                // The email/password fields are those of your account.
                // The name field will be used when generating the ID of your token.
                // TODO: this also needs to be an environment variable
        ...
        // TODO: this needs to be an input uri
        ...
        // await getSolidDataset(this._sensorContactsUri, { fetch: this._authFetch });
    ...
    private buildMqttCache() {
        for (const st of this.subscribedTopicsCache) {
            ...
            targetMqttClient.on('message', (topic, payload, packet) => {
                ...
                this.counter++;
                this.shift_counter++;
                if (this.shift_counter == this.push_shift_size) {
    ...
    startSavingCycle() {
        this._emitter.addListener('MaxReached',async () => {
            ...
            try {
                const thingArr = this.dataBuffer.splice(0, this.push_shift_size);
    ...
    async destroy() {
        ...
        await fs.writeFile(`pushsplice_${this._dataUriPath.split('/')[1]}.csv`, s, 'utf8');
