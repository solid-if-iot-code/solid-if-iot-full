...
async function getSensorInboxResource(session: Session, webId: string): Promise<string | null> {
    ...
    try {
        ...
        sensorInboxUri = getStringNoLocale(extWebID!, 'http://www.example.org/sensor#sensorInbox')!;
        // 
    } catch (err) {
        sensorInboxUri = getStringNoLocale(rdfThing!, 'http://www.example.org/sensor#sensorInbox')!;
    }
