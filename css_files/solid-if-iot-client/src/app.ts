...
async function createSensorInboxUri(session: Session, sensorInboxUri: string): Promise<string> {
    ...
    try {
        ...
        let uDataset = await getSolidDatasetWithAcl(podSensorInboxUri, { fetch: session.fetch });
        console.log(uDataset);
        let aclDataset;
        // if (!hasResourceAcl(uDataset)) {
            // if (!hasFallbackAcl(uDataset)) {
            //     // @ts-ignore
            //     aclDataset = createAcl(uDataset);
            // }  else {
            //     // @ts-ignore
            //     aclDataset = createAclFromFallbackAcl(uDataset);
            // }
            ...
            aclDataset = await universalAccess.setPublicAccess(podSensorInboxUri, { read: false, append: false, write: false, controlRead: false, controlWrite: false }, { fetch: session.fetch });
            // aclDataset = setAgentResourceAccess(aclDataset, session.info.webId as string, { read: true, write: true, append: true, control: true });
            aclDataset = await universalAccess.setAgentAccess(podSensorInboxUri, session.info.webId as string, { read: true, append: true, write: true, controlRead: true, controlWrite: true }, { fetch: session.fetch });
            // @ts-ignore
            // newAccess = await saveAclFor(uDataset, aclDataset, {fetch: session.fetch});
        // }
        // console.log(`set access for ${session.info.webId as string} to ${podSensorInboxUri}`);
...
app.post('/add_sensor_contacts', upload.none(), async (req: Request, res: Response) => {
    ...
    if (session) {
        ...
        try {
            ...
            // let myAccess = await universalAccess.getAgentAccess(sensorContactsUri!, session.info.webId!, { fetch: session.fetch })
            ...
            // console.log(sensorInboxResourceUri);
            // myAccess = await universalAccess.getAgentAccess(sensorInboxResourceUri!, session.info.webId!, { fetch: session.fetch })
            // console.log(myAccess)
            // TODO: fix this when the mentally diverse belgians stabilize their API
            // const access = await universalAccess.setPublicAccess(sensorInboxResourceUri!, { read: true, append: false, write: false, controlRead: false, controlWrite: false  }, { fetch: session.fetch });
            // console.log(access);
        ...
        if (sensorInboxResource) {
            ...
            for (const sensorThingUrl of sensorThingUrls) {
                ...
                for (const sThing of sThings) {
                    ...
                    for (const topic of topicsThings) {
                        ...
                        // console.log(`subscribeable topics array`)
                        // console.log(subscribeObjects)
            ...
            if (subscribedTopics.length > 0) {
                for (const key in sensorThings) {
                    ...
                    // @ts-ignore
                    // console.log(sensorThings[key]);
                    keys.push(key)
                    // @ts-ignore
                    for (let h = 0; h < sensorThings[key].subscribeTopics.length; h++) {
                        // console.log(sensorThings[key].subscribeTopics.length);
                        for (let i = 0; i < subscribedTopics.length; i++) {
                            ...
                            // console.log(subscribedTopicsTopicUri);
                            // console.log(sensorThings[key].topicsUri);
                            const subscribedTopicsTopicName = getStringNoLocale(subscribedTopics[i], "https://www.example.org/identifier#topicName");
                            // console.log(subscribedTopicsTopicName);
                            // console.log(sensorThings[key].subscribeTopics[h].topicName);
                            // @ts-ignore
                            if (sensorThings[key].subscribeTopics[h].topicName === subscribedTopicsTopicName && sensorThings[key].topicsUri === subscribedTopicsTopicUri) {
                                ...
                                // console.log('swapped to subscribe!');
                                break;
                            }
                        }
                    }
            ...
            //     console.log(sensorThings[key].subscribeTopics)
            // }
...
app.post('/subscribe', async (req: Request, res: Response) => {
    console.log('inside of subscribe fn')
    console.log(req.body);
    //res.redirect('/home');
    const session = await getSessionFromStorage((req.session as CookieSessionInterfaces.CookieSessionObject).sessionId)
    if (session) {
        try {
            ...
            console.log(topicUri, topicName, qos, brokerUri, sensorUri);
            const subscribedTopicsUri = `${await getStorageUri(session)}public/subscribedTopics`;
...
function defaultLogoutHandler(errTemplate: string = 'error.pug', loggedOutTemplate = 'logged_out.pug') {
    return async (req: Request, res: Response) => {
        if (typeof req.session === undefined || typeof req.session === null) {
            res.render(errTemplate)
        } else {
            const session = await getSessionFromStorage((req.session as CookieSessionInterfaces.CookieSessionObject).sessionId)
            if (session?.info.isLoggedIn) {
                await session.logout();
            }
            res.render(loggedOutTemplate)
        }
    }
}

function defaultGETLoginHandler(oidcRedirectUrl = "http://localhost:3002/redirect-from-solid-idp", clientName = "SOLID Client App", errRedirectUrl = "/") {
    return async (req: Request, res: Response) => {
        ...
        try {
            ...
            res.redirect(errRedirectUrl);
        }
    }
}

app.get('/logout', defaultLogoutHandler())
app.get('/login', defaultGETLoginHandler())
